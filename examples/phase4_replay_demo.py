#!/usr/bin/env python3
"""
Phase 4: Replay Mode Demo

This demo shows how to:
1. Record agent actions to ledger
2. Replay those actions with recorded outputs
3. Detect divergence when inputs change
4. Use replay for testing without API costs

Replay mode is useful for:
- Debugging: Re-run failed sessions with recorded outputs
- Testing: Verify behavior changes without calling external APIs
- Cost Savings: Test logic without incurring API costs
- Determinism Detection: Identify non-deterministic behavior
"""
...
# ============================================================================
# Part 2: Replay with recorded outputs
# ============================================================================
    ...
    with replay_mode(ledger_path=ledger_path, strict=False) as replay:
        ...
        for i, div in enumerate(divergences, 1):
            ...
            if div['type'] == 'input_mismatch':
                ...
                print(f"      Expected: {div['expected_inputs']}")
                print(f"      Actual: {div['actual_inputs']}")
    
    print("\n💡 Lenient mode logs warnings but continues execution.")
    print("   Useful for debugging when you expect some variance.\n")


def demo_replay_partial():
    """Demo: Replay only part of a run."""
    print("=" * 70)
    print("DEMO 4: Partial Replay")
    print("=" * 70)
    ...
    # Step 1: Record multiple actions
    print("\n📝 STEP 1: Recording actions...\n")
    run_weather_agent(cities)
    
    ledger_path = Ledger.get_log_path()
    
    # Step 2: Create replay mode and manually control it
    print("🔁 STEP 2: Partial replay (first 3 actions only)...\n")
    
    replay = ReplayMode.from_ledger_file(ledger_path, strict=True)
    
    with replay:
        # Only replay first city
        print("  Replaying first city only...\n")
        run_weather_agent([cities[0]])
        
        progress = replay.get_progress()
        print(f"\n📊 Partial replay: {progress[0]}/{progress[1]} actions")
        print(f"   Remaining: {progress[1] - progress[0]} actions\n")
    
    print("💡 You can control replay granularity for focused testing.\n")


def demo_testing_without_costs():
    """Demo: Use replay for cost-free testing."""
    print("=" * 70)
    print("DEMO 5: Testing Without API Costs")
    print("=" * 70)
    
    print("\n💡 Scenario: You want to test agent logic changes without")
    print("   calling expensive APIs or incurring costs.\n")
...
def main():
    ...
    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║                                                                    ║")
    print("║               AGENT SENTINEL - PHASE 4 REPLAY DEMO                ║")
    print("║                                                                    ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
